...
fn basic_transaction_api(test_dir: &'static str) -> Result<(), libwallet::Error> {
	...
	wallet::controller::owner_single_use(
		...
		|api, m| {
			...
			let tx = txs.txs.iter().find(|t| t.tx_slate_id == Some(slate.id));
			assert!(tx.is_some());
			let tx = tx.unwrap();
			assert!(!tx.confirmed);
	...
	wallet::controller::owner_single_use(
		...
		|sender_api, m| {
			...
			let est = sender_api.init_send_tx(m, init_args, is_node_synced.clone())?;
	...
	wallet::controller::owner_single_use(
		...
		|sender_api, m| {
			...
			assert!(refreshed);
			let txs = sender_api.retrieve_txs(m, true, None, None, None, None, None)?;
...
fn tx_rollback(test_dir: &'static str) -> Result<(), libwallet::Error> {
	...
	let is_node_synced = Arc::new(AtomicBool::new(true)); // mine a few blocks
