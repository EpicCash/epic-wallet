...
pub fn build_cli() -> Command {
    Command::new("epic-wallet")
		...
		.subcommand(
			Command::new("export_tx")
				.about("Export one or all transactions to a file in CSV or JSON format.")
				.arg(Arg::new("id")
					.short('i').long("id")
					.help("Transaction ID to export (omit if using --all)").num_args(1))
				.arg(Arg::new("all")
					.long("all")
					.help("Export all transactions").action(clap::ArgAction::SetTrue))
				.arg(Arg::new("output")
					.short('o').long("output")
					.help("Output file path").num_args(1).required(true))
				.arg(Arg::new("format")
					.long("format")
					.help("Export format: csv or json")
					.value_parser(["csv", "json"]).default_value("json").num_args(1))
		)
		.subcommand(
			Command::new("import_tx")
				.about("Import transactions from a file in CSV or JSON format.")
				.arg(Arg::new("input")
					.short('i').long("input")
					.help("Input file path").num_args(1).required(true))
				.arg(Arg::new("format")
					.long("format")
    ...
    ),
{
    ...
    let res = match wallet_args.subcommand() {
        Some(("export_tx", args)) => {
            ...
            let output = args.get_one::<String>("output").cloned().unwrap();
            let format = args
                .get_one::<String>("format")
                .cloned()
                .unwrap_or_else(|| "json".to_string());

            let export_args = command::ExportTxArgs {
                id,
                all,
                output,
                format,
            ...
            let km = (&keychain_mask).as_ref();
            match command::export_tx(wallet, km, export_args, is_node_synced.clone()) {
                Ok(_) => Ok("export_tx".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("import_tx", args)) => {
            // Parse CLI args
            let input = args.get_one::<String>("input").cloned().unwrap();
            let format = args
                .get_one::<String>("format")
                .cloned()
                .unwrap_or_else(|| "json".to_string());

            let import_args = command::ImportTxArgs { input, format };
            let km = (&keychain_mask).as_ref();
            match command::import_tx(wallet, km, import_args, is_node_synced.clone()) {
                Ok(_) => Ok("import_tx".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("init", args)) => {
            ...
            match command::init(wallet, &global_wallet_args, a) {
                Ok(_) => Ok("init".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("recover", _)) => {
            let a = arg_parse!(parse_recover_args(&global_wallet_args,));
            match command::recover(wallet, a) {
                Ok(_) => Ok("recover".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("listen", args)) => {
        ...
        Some(("owner_api", args)) => {
            ...
            match command::owner_api(
                ...
                Ok(_) => Ok("owner_api".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("web", _)) => match command::owner_api(
            wallet,
            keychain_mask,
            &wallet_config,
            &tor_config,
            ...
            Ok(_) => Ok("web".to_string()),
            Err(e) => Err(e),
        },
        Some(("account", args)) => {
            let a = arg_parse!(parse_account_args(&args));
            match command::account(wallet, km, a, is_node_synced.clone()) {
                Ok(_) => Ok("account".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("send", args)) => {
        ...
        Some(("receive", args)) => {
            ...
            match command::receive(wallet, km, &global_wallet_args, a) {
                Ok(_) => Ok("receive".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("finalize", args)) => {
            let a = arg_parse!(parse_finalize_args(&args));
            match command::finalize(wallet, km, a, is_node_synced.clone()) {
                Ok(_) => Ok("finalize".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("invoice", args)) => {
            let a = arg_parse!(parse_issue_invoice_args(&args));
            match command::issue_invoice_tx(wallet, km, a, is_node_synced.clone()) {
                Ok(_) => Ok("invoice".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("pay", args)) => {
            let a = arg_parse!(parse_process_invoice_args(&args, !test_mode));
            match command::process_invoice(
                wallet,
                km,
                a,
                wallet_config.dark_background_color_scheme.unwrap_or(true),
        ...
        Some(("info", args)) => {
            ...
            match command::info(
                ...
                Ok(_) => Ok("info".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("outputs", args)) => {
            let a = arg_parse!(parse_outputs_args(&args));
            match command::outputs(
                wallet,
                km,
                &global_wallet_args,
                a,
        ...
        Some(("txs", args)) => {
            ...
            match command::txs(
                ...
                Ok(_) => Ok("txs".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("post", args)) => {
            let a = arg_parse!(parse_post_args(&args));
            match command::post(wallet, km, a, is_node_synced.clone()) {
                Ok(_) => Ok("post".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("repost", args)) => {
            let a = arg_parse!(parse_repost_args(&args));
            match command::repost(wallet, km, a, is_node_synced.clone()) {
                Ok(_) => Ok("repost".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("cancel", args)) => {
            let a = arg_parse!(parse_cancel_args(&args));
            match command::cancel(wallet, km, a, is_node_synced.clone()) {
                Ok(_) => Ok("cancel".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("export_proof", args)) => {
            let a = arg_parse!(parse_export_proof_args(&args));
            match command::proof_export(wallet, km, a, is_node_synced.clone()) {
                Ok(_) => Ok("export_proof".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("verify_proof", args)) => {
            let a = arg_parse!(parse_verify_proof_args(&args));
            match command::proof_verify(wallet, km, a, is_node_synced.clone()) {
                Ok(_) => Ok("verify_proof".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("address", _)) => match command::address(
            wallet,
            &global_wallet_args,
            km,
            epicbox_config,
        ...
        Some(("scan", args)) => {
            ...
            match command::scan(wallet, km, a, is_node_synced.clone()) {
                Ok(_) => Ok("scan".to_string()),
                Err(e) => Err(e),
            }
        }
        Some(("change_password", args)) => {
            ...
            let mut wallet_lock = wallet.lock();
            let lc = wallet_lock.lc_provider().unwrap();
            match lc.change_password(None, old_password, new_password, remove_backup) {
                Ok(_) => Ok("change_password".to_string()),
                Err(e) => Err(e),
            }
        }
