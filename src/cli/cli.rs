...
use epic_wallet_controller::command::GlobalArgs;
use epic_wallet_controller::Error;
use epic_wallet_impls::DefaultWalletImpl;
use epic_wallet_libwallet::{NodeClient, StatusMessage, WalletInst, WalletLCProvider};
...
const COLORED_PROMPT: &'static str = "\x1b[36mepic-wallet>\x1b[0m ";
const PROMPT: &'static str = "epic-wallet> ";
	...
	K: keychain::Keychain + 'static,
{
	...
	let yml = load_yaml!("../bin/epic-wallet.yml");
	...
	loop {
		match reader.readline(PROMPT) {
			Ok(command) => {
				...
				// Just add 'epic-wallet' to each command behind the scenes
				// so we don't need to maintain a separate definition file
				let augmented_command = format!("epic-wallet {}", command);
				let args =
