...
OUTPUTPATH=release
BINARY=./target/release/epic-wallet
...
cp ./etc/README.MD $BINARY ./$OUTPUTPATH/$NAME
cd ./$OUTPUTPATH && tar -czvf $TARFILE epic_wallet/ && cd ../
rm -r ./$OUTPUTPATH/$NAME/

# Generate the deb file and tar
mkdir -p ./$OUTPUTPATH/$NAME
fakeroot make -f debian/rules binary
cd ./$OUTPUTPATH && tar -czvf $DEBTARFILE epic_wallet/ && cd ../
rm -r ./$OUTPUTPATH/$NAME
